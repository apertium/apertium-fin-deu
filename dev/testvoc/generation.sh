...
or:    $0 --hfst lang1-lang2 foo-bar.automorf.hfst
...
if $HFST; then
    if [[ ${dix} = guess ]]; then
        dix="${mode}".automorf.hfst
    fi
    analysis_expansion_hfst "${dix}" "${clb}" \
        | parallel -j3 --pipe --block 10M -- bash -c \'mode_after_tagger modes/"${mode}".mode\' \
        | only_errs
else
    if [[ ${dix} = guess ]]; then
